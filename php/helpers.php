...
/* helper functions */
function isUser() {
  return (_ADMIN!==true)?"YES":"NO";
}

function entity($text) {
  $to_ncr = array(
    #cleaning up the XHTML output
    '//"'                       => '"',
    "".'"'.">\r\n"              => "\">",
    '<b>'                       => '<strong>',
    '</b>'                      => '</strong>',
    ...
    # Textual entities to numerical entities
...
function l($var) {
  global $l;
  if(array_key_exists($var,$l)) return entity($l[$var]);
  return false;
}
function include_language_file($subdirred) {
  $prfx = '';
  for($x=0;$x<$subdirred;$x++) {
    $prfx .= '../';
  }
  if(file_exists($prfx.'lang/'.s('language').'.php') === true) {
    include($prfx.'lang/'.s('language').'.php');
  } else {
    include($prfx.'lang/EN.php');
  }
}
function s($var) {
...
function connect_to_db() {
  $db = mysqli_connect(db('dbhost'), db('dbuname'), db('dbpass'), db('dbname'));
  return $db;
}
class Catinfo {
   public $id;
...
function getCat($catID) {
  ...
  $catQ = "SELECT  name,	seftitle,	description,	published,	catorder,	subcat FROM "._PRE."categories WHERE id = ?";
  $stmt = $mysqli->prepare("SELECT id, name,	seftitle,	description,	published,	catorder,	subcat FROM "._PRE."categories WHERE id = ?");
  $stmt->bind_param('i', $catID);
  $stmt->execute();
  $stmt->bind_result($id, $name, $seftitle, $description, $published, $catorder, $subcat);
  if($stmt->fetch()) {
    $CatinfoInstance = new Catinfo();
    $CatinfoInstance->id=$id;
    $CatinfoInstance->name=$name;
    $CatinfoInstance->seftitle=$seftitle;
    $CatinfoInstance->description=$description;
    $CatinfoInstance->published=$published;
    $CatinfoInstance->catorder=$catorder;
    $CatinfoInstance->subcat=$subcat;
    return $CatinfoInstance;
  } else
    return false;
}
/**
