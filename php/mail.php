...
class email
{
  ...
  public static function send($head, $subject, $body, $attachments=array())
  {
    ...
    foreach($head['to'] as $cc_addr=>$cc_name)
        $to.= $cc_name . "<" . $cc_addr . ">";
    $fromaddress_keys = array_keys($head['from']);
    $fromaddress = array_pop($fromaddress_keys);
    $fromaddress_values = array_values($head['from']);
    $fromname = array_pop($fromaddress_values);
    $eol="\r\n";
    $mime_b = md5(time());
    $mime_boundary = $mime_b."-1";
    $mime_boundary2= $mime_b."-2";
    ...
    $headers .= "E-mail software does not support MIME-formatted messages.";
    ...
    $msg .= "Content-Type: text/plain; charset=UTF-8; format=flowed".$eol;
    ...
    $msg .= "<html>".$eol;
    $msg .= "<body>".$eol;
    $msg .= email::mime_html_encode($body).$eol;
    $msg .= "</body>".$eol;
    $msg .= "</html>".$eol;
    $msg .= $eol.$eol;
    ...
    for($i=0; $i < count($attachments); $i++)
    {
        ...
        {  
          ...
          $mime_type = email::get_mimetype( array_pop( explode(".", $attachments[$i] ) ) );
  ...
  {
    if (count(email::$mimetypes)==0)
        email::$mimetypes = email::mimelist();
    $ext_lower = strtolower(str_replace(".","",$ext));
    if (isset( email::$mimetypes[$ext_lower] ))
        return email::$mimetypes[$ext_lower];
